...
#include <bm/bluetooth/services/ble_hrs_c.h>
...
LOG_MODULE_REGISTER(ble_hrs_c, 3);
...
{
	struct ble_hrs_c *ble_hrs_c = (struct ble_hrs_c *)req->ctx;
	struct ble_hrs_c_evt evt = {
		.evt_type = BLE_HRS_C_EVT_ERROR,
	...
	if (ble_hrs_c->evt_handler) {
		ble_hrs_c->evt_handler(ble_hrs_c, &evt);
	}
}

static void on_hvx(struct ble_hrs_c *ble_hrs_c, const ble_evt_t *ble_evt)
{
	uint32_t index = 0;
	const ble_gattc_evt_hvx_t *hvx = &ble_evt->evt.gattc_evt.params.hvx;
	struct ble_hrs_c_evt ble_hrs_c_evt = {
		.evt_type = BLE_HRS_C_EVT_HRM_NOTIFICATION,
		.conn_handle = ble_hrs_c->conn_handle,
		.params.hrm.rr_intervals_cnt = 0,
	};

	/* Check if the event is on the link for this instance. */
	if (ble_hrs_c->conn_handle != ble_evt->evt.gattc_evt.conn_handle) {
	...
	if (hvx->handle != ble_hrs_c->peer_hrs_db.hrm_handle) {
		return;
	}

	LOG_DBG("Received HVX on link 0x%x, hrm_handle 0x%x",
		hvx->handle, ble_hrs_c->peer_hrs_db.hrm_handle);
	...
	if ((hvx->data[0] & HRM_FLAG_MASK_HR_RR_INT)) {
		ble_hrs_c_evt.params.hrm.rr_intervals_cnt = CONFIG_BLE_HRS_C_RR_INTERVALS_MAX_COUNT;

		for (uint32_t i = 0; i < CONFIG_BLE_HRS_C_RR_INTERVALS_MAX_COUNT; i++) {
			if (index >= hvx->len) {
	...
	ble_hrs_c->evt_handler(ble_hrs_c, &ble_hrs_c_evt);
}

static void on_disconnected(struct ble_hrs_c *ble_hrs_c, const ble_evt_t *ble_evt)
{
	if (ble_hrs_c->conn_handle == ble_evt->evt.gap_evt.conn_handle) {
		ble_hrs_c->conn_handle = BLE_CONN_HANDLE_INVALID;
		ble_hrs_c->peer_hrs_db.hrm_cccd_handle = BLE_GATT_HANDLE_INVALID;
		ble_hrs_c->peer_hrs_db.hrm_handle = BLE_GATT_HANDLE_INVALID;
	}
}

void ble_hrs_on_db_disc_evt(struct ble_hrs_c *ble_hrs_c, const struct ble_db_discovery_evt *evt)
{
	const struct ble_gatt_db_char *db_char;
	struct ble_hrs_c_evt hrs_c_evt = {
		.evt_type = BLE_HRS_C_EVT_DISCOVERY_COMPLETE,
	...
	if (evt->evt_type == BLE_DB_DISCOVERY_COMPLETE &&
		 ...
		 */
		if (ble_hrs_c->conn_handle != BLE_CONN_HANDLE_INVALID) {
			if ((ble_hrs_c->peer_hrs_db.hrm_cccd_handle == BLE_GATT_HANDLE_INVALID) &&
			    (ble_hrs_c->peer_hrs_db.hrm_handle == BLE_GATT_HANDLE_INVALID)) {
				ble_hrs_c->peer_hrs_db = hrs_c_evt.params.peer_db;
			}
		}

		ble_hrs_c->evt_handler(ble_hrs_c, &hrs_c_evt);
	}
}
...
uint32_t ble_hrs_c_init(struct ble_hrs_c *ble_hrs_c, struct ble_hrs_c_config *ble_hrs_c_init)
{
	...
	if (!ble_hrs_c || !ble_hrs_c_init) {
		return NRF_ERROR_NULL;
	}

	ble_hrs_c->evt_handler = ble_hrs_c_init->evt_handler;
	ble_hrs_c->gatt_queue = ble_hrs_c_init->gatt_queue;
	ble_hrs_c->conn_handle = BLE_CONN_HANDLE_INVALID;
	ble_hrs_c->peer_hrs_db.hrm_cccd_handle = BLE_GATT_HANDLE_INVALID;
	ble_hrs_c->peer_hrs_db.hrm_handle = BLE_GATT_HANDLE_INVALID;

	return ble_db_discovery_service_register(ble_hrs_c_init->db_discovery, &hrs_uuid);
}

void ble_hrs_c_on_ble_evt(ble_evt_t const *ble_evt, void *ctx)
{
	struct ble_hrs_c *ble_hrs_c = (struct ble_hrs_c *)ctx;

	if (!ble_hrs_c || !ble_evt) {
		return;
	}

	switch (ble_evt->header.evt_id) {
	case BLE_GATTC_EVT_HVX:
		on_hvx(ble_hrs_c, ble_evt);
		break;
	case BLE_GAP_EVT_DISCONNECTED:
		on_disconnected(ble_hrs_c, ble_evt);
		break;
	default:
		break;
	}
...
static uint32_t cccd_configure(struct ble_hrs_c *ble_hrs_c, bool enable)
{
	LOG_DBG("Configuring CCCD. CCCD Handle = %d, Connection Handle = %d",
		ble_hrs_c->peer_hrs_db.hrm_cccd_handle, ble_hrs_c->conn_handle);
	...
	struct ble_gq_req hrs_c_req = {
		...
		.ctx = ble_hrs_c,
		.gattc_write.handle = ble_hrs_c->peer_hrs_db.hrm_cccd_handle,
		.gattc_write.len = BLE_CCCD_VALUE_LEN,
		.gattc_write.p_value = cccd,
		.gattc_write.write_op = BLE_GATT_OP_WRITE_REQ,
	};

	return ble_gq_item_add(ble_hrs_c->gatt_queue, &hrs_c_req, ble_hrs_c->conn_handle);
}
...
uint32_t ble_hrs_c_hrm_notif_enable(struct ble_hrs_c *ble_hrs_c)
{
	...
	return cccd_configure(ble_hrs_c, true);
}
				  ...
				  const struct hrs_db *p_peer_hrs_handles)
{
	...
	ble_hrs_c->conn_handle = conn_handle;

	if (p_peer_hrs_handles) {
		ble_hrs_c->peer_hrs_db = *p_peer_hrs_handles;
	}

	return ble_gq_conn_handle_register(ble_hrs_c->gatt_queue, conn_handle);
}
/** @} */
