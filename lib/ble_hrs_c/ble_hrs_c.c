...
#define HRM_FLAG_MASK_HR_16BIT                                                                     \
	(0x01 << 0) /**< Bit mask used to extract the type of heart rate value. This is used to    \
		       find if the received heart rate is a 16 bit value or an 8 bit value. */
#define HRM_FLAG_MASK_HR_RR_INT                                                                    \
	(0x01 << 4) /**< Bit mask used to extract the presence of RR_INTERVALS. This is used to    \
 ...
 */
static void gatt_error_handler(uint16_t conn_handle, uint32_t nrf_error, void *p_ctx)
{
	struct ble_hrs_c *p_ble_hrs_c = (struct ble_hrs_c *)p_ctx;
 ...
 *            and checks whether it is a notification of the heart rate measurement from the peer.
 * If it is, this function decodes the heart rate measurement and sends it to the application.
 *
 * @param[in] p_ble_hrs_c Pointer to the Heart Rate Client structure.
 * @param[in] p_ble_evt   Pointer to the BLE event received.
 */
static void on_hvx(struct ble_hrs_c *p_ble_hrs_c, const ble_evt_t *p_ble_evt)
{
	...
	LOG_DBG("Received HVX on link 0x%x, hrm_handle 0x%x",
		p_ble_evt->evt.gattc_evt.params.hvx.handle, p_ble_hrs_c->peer_hrs_db.hrm_handle);
		...
		if (!(p_ble_evt->evt.gattc_evt.params.hvx.data[index++] & HRM_FLAG_MASK_HR_16BIT)) {
			...
			ble_hrs_c_evt.params.hrm.hr_value =
				p_ble_evt->evt.gattc_evt.params.hvx
					.data[index++]; // lint !e415 suppress Lint Warning 415:
							// Likely access out of bond
		} else {
			// 16-bit heart rate value received.
			ble_hrs_c_evt.params.hrm.hr_value =
				(((uint16_t)((uint8_t *)(&(
					p_ble_evt->evt.gattc_evt.params.hvx.data[index])))[0])) |
				(((uint16_t)((uint8_t *)(&(
					 p_ble_evt->evt.gattc_evt.params.hvx.data[index])))[1])
				 << 8);
		...
		if ((p_ble_evt->evt.gattc_evt.params.hvx.data[0] & HRM_FLAG_MASK_HR_RR_INT)) {
			...
			for (i = 0; i < BLE_HRS_C_RR_INTERVALS_MAX_CNT; i++) {
				...
				ble_hrs_c_evt.params.hrm.rr_intervals[i] =
					(((uint16_t)((
						uint8_t *)(&(p_ble_evt->evt.gattc_evt.params.hvx
								     .data[index])))[0])) |
					(((uint16_t)((uint8_t *)(&(p_ble_evt->evt.gattc_evt.params
									   .hvx.data[index])))[1])
					 << 8);
				index += sizeof(uint16_t);
			}
 ...
 *            associated with the current instance of the module. If the event is happening, the
 * function sets the instance's conn_handle to invalid.
 *
 * @param[in] p_ble_hrs_c Pointer to the Heart Rate Client structure.
 * @param[in] p_ble_evt   Pointer to the BLE event received.
 */
static void on_disconnected(struct ble_hrs_c *p_ble_hrs_c, const ble_evt_t *p_ble_evt)
{
...
void ble_hrs_on_db_disc_evt(struct ble_hrs_c *p_ble_hrs_c, const struct ble_db_discovery_evt *p_evt)
{
...
{
	struct ble_hrs_c *p_ble_hrs_c = (struct ble_hrs_c *)p_context;
 ...
 */
static uint32_t cccd_configure(struct ble_hrs_c *p_ble_hrs_c, bool enable)
{
	LOG_DBG("Configuring CCCD. CCCD Handle = %d, Connection Handle = %d",
		p_ble_hrs_c->peer_hrs_db.hrm_cccd_handle, p_ble_hrs_c->conn_handle);

	struct ble_gq_req hrs_c_req = {0};
	uint8_t cccd[BLE_CCCD_VALUE_LEN];
	uint16_t cccd_val = enable ? BLE_GATT_HVX_NOTIFICATION : 0;

	cccd[0] = cccd_val & 0x00FF;
	cccd[1] = (cccd_val & 0xFF00) >> 8;

	hrs_c_req.type = BLE_GQ_REQ_GATTC_WRITE;
	hrs_c_req.error_handler.cb = gatt_error_handler;
	hrs_c_req.error_handler.ctx = p_ble_hrs_c;
	hrs_c_req.gattc_write.handle = p_ble_hrs_c->peer_hrs_db.hrm_cccd_handle;
	hrs_c_req.gattc_write.len = BLE_CCCD_VALUE_LEN;
	hrs_c_req.gattc_write.p_value = cccd;
	hrs_c_req.gattc_write.write_op = BLE_GATT_OP_WRITE_REQ;

	return ble_gq_item_add(p_ble_hrs_c->p_gatt_queue, &hrs_c_req, p_ble_hrs_c->conn_handle);
}
				  ...
				  const hrs_db_t *p_peer_hrs_handles)
{
	...
	return ble_gq_conn_handle_register(p_ble_hrs_c->p_gatt_queue, conn_handle);
}
/** @}
 *  @endcond
 */
